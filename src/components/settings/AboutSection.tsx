    ...
    try {
      ...
      if (!displayVersion) {
        await settingsApi.openExternal(
          "https://github.com/Barrierml/tako-switch/releases",
        );
        return;
      }

      await settingsApi.openExternal(
        `https://github.com/Barrierml/tako-switch/releases/tag/${displayVersion}`,
      );
    } catch (error) {
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <img src={appIcon} alt="Tako Switch" className="h-5 w-5" />
              <h4 className="text-lg font-semibold text-foreground">
                Tako Switch
              </h4>
            </div>
            <p className="text-xs text-muted-foreground">
              {t("settings.forkCredit")}
            </p>
            <div className="flex items-center gap-2">
            ...
            <Button
              ...
              onClick={() =>
                settingsApi.openExternal(
                  "https://github.com/Barrierml/tako-switch",
                )
              }
              className="h-8 gap-1.5 text-xs"
            >
