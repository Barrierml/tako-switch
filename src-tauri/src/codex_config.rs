...
pub const CC_SWITCH_CODEX_MODEL_CATALOG_FILENAME: &str = "tako-switch-model-catalog.json";
...
/// `tako-switch-model-catalog.json`). A user-managed external catalog file is
...
name = "any"
"#;
        let catalog_path = Path::new("/tmp/tako-switch-model-catalog.json");
    ...
    fn resolve_catalog_path_returns_none_when_config_missing_field() {
        let generated = PathBuf::from("/tmp/.codex/tako-switch-model-catalog.json");
    ...
    fn resolve_catalog_path_accepts_cc_switch_owned_file() {
        let generated = PathBuf::from("/tmp/.codex/tako-switch-model-catalog.json");
        let config = r#"model_catalog_json = "/tmp/.codex/tako-switch-model-catalog.json"
"#;
...
"#;
        ...
        let unc_path =
            Path::new(r"\\wsl.localhost\Ubuntu\home\user\.codex\tako-switch-model-catalog.json");
...
model = "glm-5"
"#;
        let regular_path = Path::new("/home/user/.codex/tako-switch-model-catalog.json");
    ...
    fn set_catalog_json_none_removes_cc_switch_owned_by_filename() {
        ...
        let input = r#"model_catalog_json = "/home/user/.codex/tako-switch-model-catalog.json"
"#;
        let result = set_codex_model_catalog_json_field(input, None).unwrap();
        let parsed: toml::Value = toml::from_str(&result).unwrap();
        assert!(
            parsed.get("model_catalog_json").is_none(),
            "None arm should remove tako-switch-owned field regardless of path format"
        );
    }
...
model_catalog_json = "tako-switch-model-catalog.json"
"#;
        let generated_path = PathBuf::from("/home/user/.codex/tako-switch-model-catalog.json");
...
"#;
    ...
    fn set_catalog_json_none_removes_relative_path() {
        let input = r#"model_catalog_json = "tako-switch-model-catalog.json"
"#;
        let result = set_codex_model_catalog_json_field(input, None).unwrap();
        let parsed: toml::Value = toml::from_str(&result).unwrap();
        assert!(
            parsed.get("model_catalog_json").is_none(),
            "None arm should remove relative tako-switch-owned field"
        );
    }
}
