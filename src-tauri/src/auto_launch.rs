...
/// 将 `/path/to/Tako Switch.app/Contents/MacOS/Tako Switch` 转换为 `/path/to/Tako Switch.app`
#[cfg(target_os = "macos")]
...
mod tests {
    ...
    fn test_get_macos_app_bundle_path_valid() {
        ...
        assert_eq!(
            result,
            Some(std::path::PathBuf::from("/Applications/Tako Switch.app"))
        );
    }
    ...
    fn test_get_macos_app_bundle_path_with_spaces() {
        let exe_path =
            std::path::Path::new("/Users/test/My Apps/Tako Switch.app/Contents/MacOS/Tako Switch");
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(
            result,
            Some(std::path::PathBuf::from(
                "/Users/test/My Apps/Tako Switch.app"
            ))
        );
    }
